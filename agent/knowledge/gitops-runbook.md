...
1. Scenario B path: use only Code Access to create a branch, edit
   `infra/leak.auto.tfvars`, commit that one-file change, and open an unmerged
   Pull Request. The platform prefixes the title with `[Generated by SRE Agent]`.
2. Scenario C path: use Code Access to do the same — create a branch, edit
   `infra/leak.auto.tfvars`, commit, and open an unmerged Pull Request. The
   platform prefixes the title with `[Generated by SRE Agent]`.
3. The Pull Request changes only `infra/leak.auto.tfvars`:
...
In both scenarios the agent holds Reader on Azure and cannot mutate the
deployment. If asked to restart or scale the app directly, decline and open the
Pull Request instead. Never approve or merge your own Pull Request.
