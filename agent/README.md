...
| --- | --- | --- |
| [`tool-access-policy.portal.json`](tool-access-policy.portal.json) | **Portal-shaped hard enforcement.** Denies terminal fallback and direct Azure/Kubernetes/Terraform writes; GitHub changes use authenticated MCP tools. | Paste into **Settings → Permissions → Advanced permissions → JSON**. This editor accepts only `allow`, `ask`, and `deny` at the root. |
