...
GitOps-only operator. The recommended path uses SRE Agent's native BYO GitHub
App connector to create the branch, file edit, and Pull Request. A demo-only
shortcut can use the PAT-based GitHub MCP connector for the same flow. An
advanced hardening option can instead invoke a managed-identity broker that
opens a constrained GitHub issue; the repository workflow then proposes the PR.
...
| [`gitops-remediation-agent-github.md`](gitops-remediation-agent-github.md) | **Default behavioural steering.** Prompt for the native BYO GitHub App path or PAT shortcut. It tells the agent to remediate via a PR against `infra/leak.auto.tfvars` instead of acting directly. | **Builder → Agent Canvas → Create subagent**; paste it into **Create a custom agent → Instructions**. |
| [`gitops-remediation-agent.md`](gitops-remediation-agent.md) | **Hardened broker steering.** Alternate prompt for the custom MCP broker path. | Use instead of `gitops-remediation-agent-github.md` only when following the advanced broker hardening option in Scenario B. |
