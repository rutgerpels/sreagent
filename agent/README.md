...
integration, and the managed-identity broker/API for no-PAT PR creation.
...
| [`gitops-remediation-agent.md`](gitops-remediation-agent.md) | **Scenario C / broker steering.** Prompt for the managed-identity broker/API path. | Use for the private-network no-PAT path. |
...
- `RunInTerminal` is intentionally denied. PR creation must come from the
  selected GitHub MCP tools in Scenario B or the broker/API tools in Scenario C,
  not from shell, `git`, or `gh`.
...
With the Scenario C broker/API path, the middle step is
